              ...
              >
                <Grid item md={3.8}>
                  ...
                  >
                    ...
                    <Box>
                      ...
                      >
                       {" "} {data?.segments.map((segment,indx)=><span> {indx!==0&&','} {segment?.operatingcareer} {segment?.operatingflight}</span>)}
                      </Typography>
                    </Box>
                  </Box>
                </Grid>
                ...
                >
                  <Box sx={{ textAlign: "center" }}>
                    ...
                    >
                      {moment(data?.departureDate).format('DD MMM YY')}
                    </Typography>
                  </Box>
                </Grid>
                ...
                >
                  <Box sx={{ textAlign: "left" }}>
                    ...
                    <Typography sx={{ fontWeight: "14px", fontWeight: "500" }}>
                      {data?.segment==='1'&&'Direct Flight'||data?.segment==='2'&&'On Stop Flight'||data?.segment==='3'&&'2 Stop Flight'}
                    </Typography>
                  </Box>
                </Grid>
              </Grid>
            ...
            <Grid item md={3}>
              <Box
                sx={{
                  ...
                  pb: 1.3,
                  borderRadius: "9px",
                  borderLeft: "3px dashed var( --darkgrey-color)",
                }}
              >
                <Box sx={{ textAlign: "right",pl:3 }}>
                  ...
                  <Typography sx={{ fontSize: "15px", fontWeight: "500" ,color:data?.refundable==='Refundable'?"var(--green-color)":'var(--red-color)'}}>
                  {data?.refundable}
                  </Typography>
                  <Typography sx={{ fontSize: "15px", fontWeight: "500" }}>
                  {data?.seat} Seats Available
                  </Typography>
...
<Accordion expanded={openDetails} sx={{boxShadow:"none", borderRadius: "20px",}}>
        <AccordionSummary
        sx={{display:"none",}}
...
<Grid item md={8}>
    <Box sx={{width:"100%",bgcolor:"#F9FAFE",borderRadius:"9px"}}>
    <Box sx={{ width: '100%', typography: 'body1' }}>
      <TabContext value={value}>
        <Box >
          <TabList onChange={handleChange} aria-label="lab API tabs example"  style={{ justifyContent: 'flex-start' }}>
            <Tab label={<Box sx={{textAlign:'left'}}>
              <Typography sx={{fontSize:"13px",textTransform:"capitalize"}}>Departure</Typography>
        <Box sx={{display:"flex",justifyContent:'start',alignItems:'center',gap:"8px"}}>
        <Typography sx={{fontSize:"18px",fontWeight:"600"}}>{data?.departure}</Typography>
<AirplanemodeActiveIcon sx={{transform:"rotate(90deg)"}}/>
        <Typography sx={{fontSize:"18px",fontWeight:"600"}}>{data?.arrival}</Typography>
        </Box>
            </Box>} value="1" sx={{ flex: 1,alignItems: 'start',borderRight:'2px solid var(  --medium-gray-color)',borderBottom:'2px solid var(  --medium-gray-color)' }}  />
            <Tab  label="Fare Policy" value="2"  sx={{textTransform:"capitalize",flex: 1,alignItems: 'start',borderBottom:'2px solid var(  --medium-gray-color)'  }} />
...
<Typography sx={{mb:1,fontSize:'13px',fontWeight:"500",color:"var(--mate-color)"}}>{segment?.flightduration}</Typography>
        ...
        {index<data?.segments.length-1&&<Box sx={{bgcolor:"rgb(224,231,239)",textAlign:'center',fontSize:'12px',py:0.4,my:2}}>
Change plane at {segment?.arrivalAirport}   ||  {segment?.arrivalLocation}   ||  Transit Time : {data?.transit[`transit${index+1}`]}
</Box>
}
        ...
        <TabPanel value="2">      <FarePolicy/></TabPanel>
...
{data?.pricebreakdown?.map((prices,priceIndx)=>    
       <Box sx={{display:"flex",justifyContent:'space-between',alignItems:'center',mb:1}}>
...
<Box sx={{display:"flex",justifyContent:'space-between',alignItems:'center',py:1,borderTop:"1px solid var(  --medium-dark-blue) ",borderBottom:"1px solid var(  --medium-dark-blue) "}}>
        ...
        <Typography sx={{fontWeight:"500",color:"var(--mate-color)"}}>BDT {data?.pricebreakdown.reduce((acc, item) => {
  return acc + parseInt(item.Tax);
      ...
      <Typography sx={{fontWeight:"500",color:"var(  --medium-dark-blue)",fontSize:"12px"}}> {data?.pricebreakdown.reduce((acc, item) => {
  return acc + parseInt(item.PaxCount);
        ...
        <TabPanel value="2">
   {data?.pricebreakdown?.map((prices)=> <Box sx={ { pb:1,borderBottom:"1px solid var(  --medium-dark-blue) ",display:"flex",justifyContent:"space-between",alignItems:'center'}}>
      <Box>
